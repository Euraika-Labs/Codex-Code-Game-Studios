...
a **confirm/adjust** prompt using `ask the user directly in plain text` — not open-ended questions.
...
decisions remain, present each one as a separate `ask the user directly in plain text` with the proposed
...
4.5. **Engine Specialist Validation** — Before saving, spawn the **primary engine specialist** as a subagent to validate the drafted ADR:
   ...
   - Spawn `custom agent: [primary specialist]` with: the ADR's Engine Compatibility section, Decision section, Key Interfaces, and the engine reference docs path. Ask them to:
...
4.6. **Technical Director Strategic Review** — After the engine specialist validation, spawn `technical-director` as a subagent using gate **TD-ADR** (`docs/studio/director-gates.md`):
...
5. **Write approval** — Use `ask the user directly in plain text`:
...
Ask using `ask the user directly in plain text`:
...
After the ADR is written (and registry optionally updated), close with `ask the user directly in plain text`.
...
> To validate ADR coverage against your GDDs, open a **fresh Codex CLI session**
