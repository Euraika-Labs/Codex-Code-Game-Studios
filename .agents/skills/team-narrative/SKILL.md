...
> Usage: `$team-narrative [narrative content description]` — describe the story content, scene, or narrative area to work on (e.g., `boss encounter cutscene`, `faction intro dialogue`, `tutorial narrative`). Do not use `ask the user directly in plain text` here; output the guidance directly.
...
- `custom agent: narrative-director` — Story arcs, character design, narrative vision
- `custom agent: writer` — Dialogue writing, lore entries, in-game text
- `custom agent: world-builder` — World rules, faction design, history, geography
- `custom agent: art-director` — Character visual profiles, environmental visual storytelling, cinematic tone
- `custom agent: level-designer` — Level layouts that serve the narrative, pacing
- `custom agent: localization-lead` — i18n validation, string key compliance, translation headroom
...
Delegate in parallel — issue all three subagent calls simultaneously before waiting for any result:
...
sub-agents spawned as a subagent. Each sub-agent enforces the "May I write to [path]?"
