...
> Usage: `$team-audio [feature or area]` — specify the feature or area to design audio for (e.g., `combat`, `main menu`, `forest biome`, `boss encounter`). Do not use `ask the user directly in plain text` here; output the guidance directly.
...
- `custom agent: audio-director` — Sonic identity, emotional tone, audio palette
- `custom agent: sound-designer` — SFX specifications, audio events, mixing groups
- `custom agent: technical-artist` — Audio middleware, bus structure, memory budgets
- `custom agent: [primary engine specialist]` — Validate audio integration patterns for the engine
- `custom agent: gameplay-programmer` — Audio manager, gameplay triggers, adaptive music
