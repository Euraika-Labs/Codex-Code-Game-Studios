...
Spawn `release-manager` as a subagent. Ask them to produce a rollback plan covering:
...
1. Spawn `lead-programmer` as a subagent with:
...
3. Spawn `qa-tester` as a subagent to verify: does the bug reproduce after the fix?
...
Spawn `qa-lead` as a subagent with:
