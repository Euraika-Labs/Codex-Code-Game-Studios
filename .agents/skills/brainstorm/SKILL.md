...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` as a subagent in parallel before moving to Phase 5. Issue both subagent calls simultaneously — do not wait for one before starting the other.**

- **`creative-director`** — gate **CD-PILLARS** (`docs/studio/director-gates.md`)
...
**After identifying biggest technical risks, spawn `technical-director` as a subagent using gate TD-FEASIBILITY (`docs/studio/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` as a subagent using gate PR-SCOPE (`docs/studio/director-gates.md`).**
...
4. **Generate the game concept document** using the template at
   `docs/studio/templates/game-concept.md`. Fill in ALL sections from the
...
If yes, generate the document using the template at `docs/studio/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
...
> to `design/gdd/game-concept.md`. Open a fresh Codex CLI session to continue
