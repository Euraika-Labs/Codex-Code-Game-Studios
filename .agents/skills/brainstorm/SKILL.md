...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` via Task in parallel before moving to Phase 5. Issue both subagent calls simultaneously — do not wait for one before starting the other.**
...
**After identifying biggest technical risks, spawn `technical-director` via Task using gate TD-FEASIBILITY (`docs/studio/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via Task using gate PR-SCOPE (`docs/studio/director-gates.md`).**
