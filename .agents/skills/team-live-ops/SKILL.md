...
**Argument check:** If no season name or event description is provided, ask the
user with a concise plain-text choice list:
> "What live-ops update should we plan?"
> `[A] I'll provide a season name`
> `[B] I'll provide an event description`
> `[C] Stop here`
>
> Example: `$team-live-ops Frostfall Season` or `$team-live-ops 2-week spring event with co-op challenges`
>
> Stop immediately after asking without spawning any subagents or reading any files.
...
- `custom agent: live-ops-designer` — Season/event structure and retention mechanics
- `custom agent: economy-designer` — Live economy balance and reward pricing
- `custom agent: analytics-engineer` — Success metrics, A/B tests, event instrumentation
- `custom agent: community-manager` — Player-facing communication and messaging
- `custom agent: narrative-director` — Seasonal theme and narrative framing
- `custom agent: writer` — All player-facing text: event descriptions, item names, copy
...
delegated to sub-agents spawned as a subagent. Each sub-agent enforces the
