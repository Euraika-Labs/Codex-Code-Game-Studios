...
Read `docs/studio/technical-preferences.md`, section `## Engine Specialists`. Note:
...
Spawn all applicable specialists simultaneously as a subagent — do not wait for one before starting the next.
...
For Logic and Integration stories, also spawn `qa-tester` as a subagent in parallel with the engine specialists. Pass:
