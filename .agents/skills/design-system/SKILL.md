...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `ask the user directly in plain text`:
...
- Read `docs/studio/technical-preferences.md` to identify the engine and version
...
Use the template structure from `docs/studio/templates/game-design-document.md`:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `ask the user directly in plain text` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `ask the user directly in plain text` to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `ask the user directly in plain text`. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use `ask the user directly in plain text` with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use `ask the user directly in plain text`:
...
spawn `creative-director` as a subagent:
...
**Agent delegation (MANDATORY)**: Before drafting Section C, spawn specialist agents as a subagent in parallel:
...
- Surface any disagreements between agents to the user via `ask the user directly in plain text`
...
**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents as a subagent in parallel:
...
- Present the specialists' proposals to the user for review via `ask the user directly in plain text`
...
**Agent delegation (MANDATORY)**: Spawn `qa-lead` as a subagent before finalising acceptance criteria. Provide: the completed GDD sections C, D, E, and ask them to validate that the criteria are independently testable and cover all core rules and formulas. Surface any gaps or untestable criteria to the user.
...
For required systems: **spawn `art-director` as a subagent** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
Before finalizing the GDD, spawn `creative-director` as a subagent using gate **CD-GDD-ALIGN** (`docs/studio/director-gates.md`).
...
> **To validate this GDD, open a fresh Codex CLI session and run:**
...
**When delegating via subagent workflow**:
...
- The main session presents the agent's output to the user via `ask the user directly in plain text`
- The user decides; the main session writes to file
...
> open a fresh Codex CLI session and run `$design-system [system-name]` — it will
