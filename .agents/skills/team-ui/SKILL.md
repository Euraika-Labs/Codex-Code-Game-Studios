...
- **engine UI specialist** — Validates UI implementation patterns against engine-specific best practices (read from `docs/studio/technical-preferences.md` Engine Specialists → UI Specialist)
...
- `custom agent: ux-designer` — User flows, wireframes, accessibility, input handling
- `custom agent: ui-programmer` — UI framework, screens, widgets, data binding
- `custom agent: art-director` — Visual style, layout polish, art bible consistency
- `custom agent: [UI engine specialist]` — Engine-specific UI pattern validation (e.g., unity-ui-specialist, ue-umg-specialist, godot-specialist)
- `custom agent: accessibility-specialist` — Accessibility compliance audit
...
Then use `ask the user directly in plain text` with options:
...
Before implementation begins, spawn the **engine UI specialist** (from `docs/studio/technical-preferences.md` Engine Specialists → UI Specialist) to review the UX spec and visual design spec for engine-specific implementation guidance:
