...
Use the subagent workflow to request sign-off in parallel:

- `custom agent: lead-programmer` — Review the fix for correctness and side effects
- `custom agent: qa-tester` — Run targeted regression tests on the affected system
- `custom agent: producer` — Approve deployment timing and communication plan
...
After approvals, determine the QA scope required before deploying the hotfix. Spawn `qa-lead` as a subagent with:
