...
2. plain-text decision prompt: user approves Phase 1 output before Phase 2 begins
...
8. plain-text decision prompt: user approves the full season plan
...
4. plain-text decision prompt presented with resolution options before season plan approval is offered
...
- [ ] plain-text decision prompt is used to surface the violation and offer resolution options (revise economy design, override with documented rationale, cancel)
...
- [ ] Error message includes the correct usage format with the metadata.short-description
...
- [ ] If economy-designer blocks but analytics-engineer succeeds, analytics output is preserved and the block is surfaced via plain-text decision prompt
...
- [ ] plain-text decision prompt used at every phase transition — user approves before the next phase begins
...
- [ ] Error recovery: any BLOCKED agent is surfaced immediately with plain-text decision prompt options (skip / retry / stop)
