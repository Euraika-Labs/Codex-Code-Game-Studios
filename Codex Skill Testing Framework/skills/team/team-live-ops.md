...
2. plain-text approval check: user approves Phase 1 output before Phase 2 begins
...
8. plain-text approval check: user approves the full season plan
...
4. plain-text approval check presented with resolution options before season plan approval is offered
...
- [ ] plain-text approval check is used to surface the violation and offer resolution options (revise economy design, override with documented rationale, cancel)
...
- [ ] If economy-designer blocks but analytics-engineer succeeds, analytics output is preserved and the block is surfaced via plain-text approval check
...
- [ ] `plain-text approval check` used at every phase transition — user approves before the next phase begins
...
- [ ] Error recovery: any BLOCKED agent is surfaced immediately with plain-text approval check options (skip / retry / stop)
