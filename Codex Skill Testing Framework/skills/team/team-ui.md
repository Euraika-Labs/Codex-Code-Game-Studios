...
Uses plain-text decision prompt at each phase transition. Delegates all file writes to
...
- [ ] Uses plain-text decision prompt at phase transitions for user approval before proceeding
...
- Engine UI specialist configured in `docs/studio/technical-preferences.md`
...
2. Phase 1b — `$ux-design inventory-screen` invoked (or ux-designer spawned directly); produces `design/ux/inventory-screen.md` using `ux-spec.md` template; plain-text decision prompt confirms spec before review
3. Phase 1c — `$ux-review design/ux/inventory-screen.md` invoked; returns APPROVED; gate passed, proceed to Phase 2
4. Phase 2 — art-director spawned; reviews full UX spec (not only wireframes); applies visual treatment; verifies color contrast; produces visual design spec with asset manifest; plain-text decision prompt confirms before Phase 3
...
4. plain-text decision prompt presented with the specific flagged concerns and options:
...
- [ ] plain-text decision prompt presents the specific flagged concerns before offering options
...
5. plain-text decision prompt presents all three review results before Phase 5 begins
...
- [ ] plain-text decision prompt shows all three review streams' results together before Phase 5 approval
...
3. plain-text decision prompt presented with options:
...
- [ ] plain-text decision prompt offers a "create pattern library first" option (referencing `$ux-design patterns`)
...
- [ ] plain-text decision prompt used at each phase transition — user approves before pipeline advances
