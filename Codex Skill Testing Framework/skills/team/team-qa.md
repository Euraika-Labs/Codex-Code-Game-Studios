...
- [ ] Uses `plain-text approval check` at phase transitions to capture user approval before proceeding
...
2. Phase 2: Spawns `qa-lead` via Task; produces strategy table classifying all 4 stories; no blockers flagged; presents to user; plain-text approval check: user selects "Looks good — proceed to test plan"
...
5. Phase 5: Spawns `qa-tester` via Task for each Visual/Feel and Integration story (2–3 stories); run in parallel; presents test cases grouped by story; plain-text approval check per group; user approves
...
2. Phase 6: User marks Visual/Feel story as FAIL; plain-text approval check collects failure description: "Animation plays at 2x speed — jitter visible on every loop"
...
**Assertions:**
- [ ] FAIL result in Phase 6 triggers plain-text approval check to collect the failure description before the bug report is written
...
2. Cannot infer sprint; uses plain-text approval check: "Which sprint or feature should QA cover?" with options to type a sprint identifier or cancel
...
- [ ] When both state files are absent, skill uses plain-text approval check rather than guessing (variant B)
...
- [ ] `plain-text approval check` used at Phase 2 (strategy review), Phase 5 (test case approval per group), and Phase 6 (per-story manual QA result)
...
- [ ] Error recovery: any BLOCKED agent is surfaced immediately with plain-text approval check options
