...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use plain-text decision prompt to capture the decision*
...
Use a plain-text decision prompt to present decisions as a simple numbered option list instead of
...
- Batch up to 4 independent questions in a single plain-text decision prompt call
...
- When running as a Task subagent (tool may not be available) — structure your
  text output so the orchestrator can present options via plain-text decision prompt
