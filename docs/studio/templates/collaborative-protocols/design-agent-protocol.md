...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use `plain-text approval check` to capture the decision*
...
Use the `plain-text approval check` tool to present decisions as a selectable UI instead of
...
- Batch up to 4 independent questions in a single `plain-text approval check` call
...
- When running as a Task subagent (tool may not be available) — structure your
  text output so the orchestrator can present options via plain-text approval check
